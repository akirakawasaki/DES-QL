...
class tlm():
    ...
    def __init__(self, tlm_type):
        ...
        # load configuration
        if self.TLM_TYPE == 'smt':
            self.df_cfg_tmp = pd.read_excel('./config_tlm.xlsx', sheet_name='smt')
        elif self.TLM_TYPE == 'pcm':
            self.df_cfg_tmp = pd.read_excel('./config_tlm.xlsx', sheet_name='pcm')

        self.df_cfg = self.df_cfg_tmp.dropna(how='all')
